<template>
  <v-toolbar :elevation="8" class="bg_opacity fixed">
    ...
    <router-link to="/" class="text-decoration-none">
      <v-app-bar-title
        class="text-white text-decoration-none font-weight-bold text-subtitle-2 text-sm-h4 text-md-h4">МАРС
    ...
    <div v-for="button of buttons" :key="button.id" class="hidden-sm-and-down">
      <router-link :to="button.link" v-smooth-scroll="{ duration: 1000 }">
        <v-btn class="ml-2" variant="text">
          <span class="font-weight-think text-white">{{ button.title }}</span>
        </v-btn>
      </router-link>
    </div>
    <v-spacer></v-spacer>
...
export default {
  data: () => ({
    drawer: false,
    buttons: [
      { title: 'Отель', link: '/hotel', id: 1 },
      { title: 'Цены', link: '/apartments/#price', id: 2 },
      { title: 'Отзывы', link: '/apartments/#testimonials', id: 3 },
      { title: 'Фотографии', link: '/apartments/#photo', id: 4 },
      { title: 'Контакты', link: '/apartments/#contacts', id: 5 }
    ]
  })
}
</script>
